...
$router -> post("/api/account/delete", function($router){
    ...
    // TODO - Remove after testing endpoint.
 ...
 *     boolean 'success'
 *     string  'message'
 *     array   'data'    - List of accounts retrived.
 */
$router -> get("/api/account/list", function($router){
    ...
    $data = [];
    foreach (load_data($config["database"]["accounts"]) as $account){
        $data[] = $account -> json(); //TODO Verify valid json is generated by this.
    }
    header("HTTP/1.0 200 OK");
    echo json_encode([
        "success" => true,
...
$router -> post("/api/kid/list", function($router){
    ...
    foreach (load_data($config["database"]["kids"]) as $kid){
        ...
        elseif ($kid -> hidden && get_self() -> has_access("mod") && $_POST["hidden"]){
            $response[] = $kid -> json();
        }
    }
    header("HTTP/1.0 200 OK");
...
$router -> post("/api/kid/add", function($router){
    ...
    add_objects([$new_kid], $kids, $config["database"]["kids"]);
    header("HTTP/1.0 200 OK");
    echo json_encode([
        "success" => true,
...
$router -> post("/api/kid/delete", function($router){
    ...
    echo json_encode([
        "success" => true,
        "message" => "Kid with ID of '" . $kid -> get_full_name() . "' deleted."
    ]);
    return;
}, ["id" => true]);
